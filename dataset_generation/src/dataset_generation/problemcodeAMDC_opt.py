...
def j0_jax(x):
    return jax.pure_callback(
        lambda v: sps.j0(v).astype(v.dtype),
...
def j1_jax(x):
    return jax.pure_callback(
        lambda v: sps.j1(v).astype(v.dtype),
        jax.ShapeDtypeStruct(x.shape, x.dtype),
        x,
        vmap_method="broadcast_all",
    )
...
def y0_jax(x):
    return jax.pure_callback(
        lambda v: sps.y0(v).astype(v.dtype),
        jax.ShapeDtypeStruct(x.shape, x.dtype),
        x,
        vmap_method="broadcast_all",
    )
...
def y1_jax(x):
    return jax.pure_callback(
        lambda v: sps.y1(v).astype(v.dtype),
        jax.ShapeDtypeStruct(x.shape, x.dtype),
        x,
        vmap_method="broadcast_all",
    )
...
def k0_jax(x):
    return jax.pure_callback(
        lambda v: sps.k0(v).astype(v.dtype),
        jax.ShapeDtypeStruct(x.shape, x.dtype),
        x,
        vmap_method="broadcast_all",
    )
...
def k1_jax(x):
    return jax.pure_callback(
        lambda v: sps.k1(v).astype(v.dtype),
        jax.ShapeDtypeStruct(x.shape, x.dtype),
        x,
        vmap_method="broadcast_all",
    )
...
_DIC_X1_FLAT = _S_grid.flatten(order="F")
_DIC_X2_FLAT = _A_grid.flatten(order="F")
...
def _precompute_A_data_jax(N):
    ...
    for k, m in km_pairs:
        ...
        a2 = np.stack(
            [_eval_lambdify(_get_alform2_func(k, m, int(lv)), R_flat) for lv in l_arr]
        )
        a3 = np.stack(
            [_eval_lambdify(_get_alform3_func(k, m, int(lv)), R_flat) for lv in l_arr]
        )

        ds = np.sign(l_arr**2 - 1)
        lgkm = sps.gammaln(k + 1.5)
        lgkm1 = sps.gammaln(k + m + 1)
        lf2k1 = sps.gammaln(2 * k + 2)
        lf2km = sps.gammaln(2 * k + 2 * m + 2)

        log_C = (
            np.log(np.pi)
            + (l_arr + 2) * np.log(2.0)
            - np.log(np.abs(l_arr**2 - 1))
            + sps.gammaln(2 * k - l_arr + 2)
            - sps.gammaln(2 * k + 2 * m + l_arr + 2)
            + lf2km
            - lf2k1
            + lgkm
            - lgkm1
            + sps.gammaln(k + l_arr / 2 + m + 1.5)
            - sps.gammaln(k - l_arr / 2 + 1)
        )
        Clmk = ds * (-1.0) ** (l_arr + 2 * m) * np.exp(log_C)

        log_E = (
            np.log(np.pi)
            + (l_arr - 2 * m + 2) * np.log(2.0)
            - np.log(np.abs(l_arr**2 - 1))
            + lgkm
            - lgkm1
            + lf2km
            - lf2k1
            + sps.gammaln(k + l_arr / 2 + 1.5)
            - sps.gammaln(m + k - l_arr / 2 + 1)
            + sps.gammaln(2 * m + 2 * k - l_arr + 2)
            - sps.gammaln(l_arr + 2 * k + 2)
        )
...
def getgl_jax(l_arr, a, b, x_nodes, w_nodes):
    gval1 = (
        a
        * b
        * (a**2 * jnp.cos(x_nodes) ** 2 + b**2 * jnp.sin(x_nodes) ** 2) ** (-1.5)
        * w_nodes
    )
...
def doubleintC_batch_jax(R_pts, THETA_pts, depth, K, a, b):
    ...
    R_dist = jnp.sqrt((x - gi) ** 2 + (y - eta) ** 2)
    X_arr = K * R_dist
    Z_scal = 2.0 * K * depth
    ...
    M_term = (K**3) * (
        (2 * Z_scal - 1) / denom2**1.5 + 3 * Z_scal**2 / denom2**2.5 + 1 / denom
    ) + (K**3) / denom

    H0 = j0_jax(X_arr) + 1j * y0_jax(X_arr)
    N_term = (K**3) * (
        2 * jnp.pi * 1j * H0 * jnp.exp(-Z_scal)
...
def _eval_phi_and_derivs(X_sol, dX_da, dX_dd, sa_j, N, a, b):
    ...
    def phi_r(s, al):
        return jnp.real(phi_series(s, al, X_sol, N))

    def phi_i(s, al):
        return jnp.imag(phi_series(s, al, X_sol, N))
...
def _compute_X_and_derivs(a, d_val, K, N, b, pre):
    ...
    size = (N + 1) ** 2
    f = 4 * jnp.pi * jnp.ones(size, dtype=jnp.complex128)
    lu, piv = jax.scipy.linalg.lu_factor(C0)
    ...
    dAM_da = jnp.real(jnp.pi * (dfinal_da * a + final))
...
def _get_jitted_evaluator(N, b, n_pts):
    def _single_eval(a, d_val, K, sa_j, pre):
        (X_sol, dX_da, dX_dd, AM, DC, dAM_da, dDC_da, dAM_dd, dDC_dd) = (
            _compute_X_and_derivs(a, d_val, K, N, b, pre)
        )
        ...
        jac_a = (
            ...
            dDC_da,
            dphi_da,
            jnp.zeros_like(dphi_dx),
            jnp.zeros_like(dphi_dy),
        )
        jac_d = (
            dAM_dd,
            dDC_dd,
...
def generate_batch_data_jax(a_vals, d_vals, K_vals, N, b, s_pts, alpha_pts):
    ...
    print(
        f"Compilation done. Evaluating {n_total} entries "
    ...
    def _drain(result):
        ...
        for j, v in enumerate(jac_a):
            jac_a_lists[j].append(np.asarray(v))
        for j, v in enumerate(jac_d):
            jac_d_lists[j].append(np.asarray(v))
