...
def lgwt(N, a, b):
    ...
    """
    # numpy's leggauss returns roots and weights for interval [-1, 1]
...
# ==========================================
# Symbolic Function Caches (For Performance)
# ==========================================
...
def _get_alform2_func(k, m, l_val):
    r = sp.Symbol('r')
    # Cast to standard Python int to avoid NumPy negative power errors
    l1 = int(m + 2*k + 1)
    m1 = int(-m + l_val)
    
    # Use -1.0 to guarantee a float result
...
def alform(k, m, s):
    ...
    res = func(s)
    # Handle the case where the derivative evaluates to a scalar constant
...
def intgralv(z, X):
    ...
    # Reshape x and w to broadcast appropriately against X
    # e.g., if X is shape (10000,), this forces x and w to (100, 1)
    ...
    # Sum over the 100 integration nodes (axis 0)
...
def check(r, theta, s, alpha, d_val, K, a, b):
    ...
    # Hankel function of the first kind (besselh defaults to H1 in MATLAB)
    N_val = (K**3) * (2 * np.pi * 1j * hankel1(0, X) * np.exp(-Z) - (4 / np.pi) * intgralv(Z, X))
...
def doubleintC(k, m, r, theta, depth, K, a, b):
    ...
    # meshgrid returning shape correctly mapped to matrix
    x, y = np.meshgrid(x1, x2, indexing='xy')
    
    # Unwrapping via Fortran column-major order equivalent to MATLAB's linear indexing
    ...
    # Transposing p4 to align with original script matrix sizes (p4.')
    integrand = w * p * p4.T
...
def getgl(l_array, a, b):
    ...
    # Broadcast to perform matrix multiplication natively
...
def hyperterm(l1_array, k, m, r, theta):
    ...
    for i, l_val in enumerate(l_arr):
        # Cast l_val to standard python int here as well!
        l_val = int(l_val) 
        
        Clmk = ( (-1.0)**(l_val + 2*m) * (np.pi * 2**(l_val + 2) / (l_val**2 - 1)) * (factorial(2*k - l_val + 1) / factorial(2*k + 2*m + l_val + 1)) * (factorial(2*k + 2*m + 1) / factorial(2*k + 1)) * (gamma(k + 1.5) / gamma(k + m + 1)) * (gamma(k + l_val/2 + m + 1.5) / gamma(k - l_val/2 + 1)) )
                 
        Elkm = ( (-1.0)**(l_val + m) * (np.pi * 2**(l_val - 2*m + 2) / (l_val**2 - 1)) * (gamma(k + 1.5) / gamma(k + m + 1)) * (factorial(2*m + 2*k + 1) / factorial(2*k + 1)) * (gamma(k + l_val/2 + 1.5) / gamma(m + k - l_val/2 + 1)) * (factorial(2*m + 2*k - l_val + 1) / factorial(l_val + 2*k + 1)) )
...
# ==========================================
# problemcodeAMDC.m (Main Execution Function)
# ==========================================
def problemcodeAMDC(N, d_val, K, a, b):
    ...
    # Equivalent to MATLAB's C \ f matrix division
