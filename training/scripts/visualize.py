...
dataset_dir = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../dataset_generation/src")
)
...
def main():
    ...
    wave_params = torch.tensor([[a_b, d_b, K] for K in K_vals], dtype=torch.float32).to(
        device
    )
    with torch.no_grad():
    ...
    ax.plot_surface(
        X_dense, Y_dense, phi_r_pred, cmap="viridis", alpha=0.8, edgecolor="none"
    )
    ax.scatter(
        x_actual,
        y_actual,
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.plot_surface(
        X_dense, Y_dense, phi_i_pred, cmap="plasma", alpha=0.8, edgecolor="none"
    )
    ax.scatter(
        x_actual,
        y_actual,
    ...
    path_consistent = os.path.join(
        os.path.dirname(__file__), "../am_dc_contour_consistent.pdf"
    )
    generate_contour_pdf(consistent_scale=True, filename=path_consistent)

    print("Saving independent scale contour plot PDF...")
    path_independent = os.path.join(
        os.path.dirname(__file__), "../am_dc_contour_independent.pdf"
    )
