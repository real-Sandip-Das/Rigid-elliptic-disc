...
def optimize_target_multistart(jax_model_fn, weights, target_idx, maximize=True, num_starts=500, steps=200):
    # Bounds for [a/b, d/b, K]
    ...
    initial_inputs = jax.random.uniform(key, shape=(num_starts, 3), minval=bounds_low, maxval=bounds_high)
    ...
    def step(carry, _):
        ...
        # Summing the batch allows JAX to compute independent gradients for all starts simultaneously
        def loss_fn(p):
        ...
        x = jnp.clip(x, bounds_low, bounds_high) # Box constraints enforced globally
        
        return (x, opt_state), loss

    # Run the entire multi-start loop in one JIT-compiled block
    (final_inputs, _), _ = jax.lax.scan(step, (initial_inputs, opt_state), None, length=steps)
    
    # Extract the absolute best input configuration
...
def main():
    ...
    cfg = WaveConfig()
    cfg.phase2_model_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../checkpoints/phase2_final.pth'))
    
    device = torch.device('cpu') # Torchax extraction usually runs on CPU
    ...
    base_model.load_state_dict(torch.load(cfg.phase2_model_path, map_location=device, weights_only=True))
    base_model.eval()
    
    # Remove spectral norm parametrizations so Torchax can lower it cleanly
    ...
    for name, target_idx, maximize in targets:
        print(f"\n--- {name} ---")
        final_x, final_output = optimize_target_multistart(jax_model_fn, weights, target_idx, maximize)
        print(f"Optimal Inputs -> a/b: {final_x[0]:.4f}, d/b: {final_x[1]:.4f}, K: {final_x[2]:.4f}")
        print(f"Predicted Coeffs -> AM: {final_output[0]:.4f}, DC: {final_output[1]:.4f}")
