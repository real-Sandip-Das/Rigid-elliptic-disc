...
def main():
    ...
    def evaluate_loader(loader, split_name):
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                abs_err_am += torch.sum(
                    torch.abs(pred_coeffs[:, 0] - true_coeffs[:, 0])
                ).item()
                abs_err_dc += torch.sum(
                    torch.abs(pred_coeffs[:, 1] - true_coeffs[:, 1])
                ).item()
    ...
    val_am, val_dc, val_mae_am, val_mae_dc, val_r2_am, val_r2_dc, val_n = (
