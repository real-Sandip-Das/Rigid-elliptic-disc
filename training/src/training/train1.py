...
def calc_first_deriv(phi, x, y):
    ...
    dphi_dx = torch.autograd.grad(
        phi, x, grad_outputs=ones, create_graph=True, retain_graph=True
    )[0]

    dphi_dy = torch.autograd.grad(
        phi, y, grad_outputs=ones, create_graph=True, retain_graph=True
    )[0]
...
def train_phase1(config):
    ...
    train_loader, val_loader = get_dataloaders(
        config.data_path, config.p1_batch_size, seed=seed
    )
    ...
    for epoch in range(start_epoch, config.p1_epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss_v = mse_loss(phi_r_pred, phi_r_true) / (
                torch.mean(phi_r_true**2) + 1e-8
            ...
            if mask.any():
                loss_d = (
                    torch.mean((dphi_r_dx[mask] - dx_r_true[mask]) ** 2)
                    / (torch.mean(dx_r_true[mask] ** 2) + 1e-8)
                    + torch.mean((dphi_r_dy[mask] - dy_r_true[mask]) ** 2)
                    / (torch.mean(dy_r_true[mask] ** 2) + 1e-8)
                    + torch.mean((dphi_i_dx[mask] - dx_i_true[mask]) ** 2)
                    / (torch.mean(dx_i_true[mask] ** 2) + 1e-8)
                    + torch.mean((dphi_i_dy[mask] - dy_i_true[mask]) ** 2)
                    / (torch.mean(dy_i_true[mask] ** 2) + 1e-8)
                )
            else:
                loss_d = 0.0
            ...
            if mask.any() and prev_avg_v < config.lra_warmup_threshold:
                ...
                grads_val = torch.autograd.grad(
                    loss_v, last_layer_weights, retain_graph=True, allow_unused=True
                )[0]
                grads_der = torch.autograd.grad(
                    loss_d, last_layer_weights, retain_graph=True, allow_unused=True
                )[0]

                max_grad_v = (
                    torch.max(torch.abs(grads_val)).item()
                    if grads_val is not None
                    else 0.0
                )
                mean_grad_d = (
                    torch.mean(torch.abs(grads_der)).item()
                    if grads_der is not None
                    else 0.0
                )
            ...
            loss.backward()
            b_norm = torch.nn.utils.clip_grad_norm_(
                model.branch.parameters(), max_norm=config.grad_clip_norm
            )
            t_norm = torch.nn.utils.clip_grad_norm_(
                model.trunk.parameters(), max_norm=config.grad_clip_norm
            )
        ...
        if epoch % max(1, log_every * 10) == 0:
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    with torch.enable_grad():
                        phi_r_pred, phi_i_pred = model.forward_trunk_only(
                            latent, x, y, a_b
                        )

                        loss_v = mse_loss(phi_r_pred, phi_r_true) / (
                            torch.mean(phi_r_true**2) + 1e-8
                        ) + mse_loss(phi_i_pred, phi_i_true) / (
                            torch.mean(phi_i_true**2) + 1e-8
                        )
            ...
            print(
                f"Epoch {epoch:5d}/{config.p1_epochs} | Validation loss: {avg_val_loss:.6f} | Relative Value Error: {avg_val_loss_v:.6f}"
            )
