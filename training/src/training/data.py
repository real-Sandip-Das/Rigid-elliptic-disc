...
class WaveDataset(Dataset):
    def __init__(self, csv_file):
        ...
        self.wave_params = torch.tensor(
            self.df[["a_b", "d_b", "wave_frequency_K"]].values, dtype=torch.float32
        )
        ...
        self.dphi_dx_real = torch.tensor(
            self.df[dx_real_cols].values, dtype=torch.float32
        )
        self.dphi_dx_imag = torch.tensor(
            self.df[dx_imag_cols].values, dtype=torch.float32
        )
        self.dphi_dy_real = torch.tensor(
            self.df[dy_real_cols].values, dtype=torch.float32
        )
        self.dphi_dy_imag = torch.tensor(
            self.df[dy_imag_cols].values, dtype=torch.float32
        )

        self.coeffs = torch.tensor(
            self.df[["Added_Mass", "Damping_Coefficient"]].values, dtype=torch.float32
        )
...
def generate_anchor_coords(a_b, N_s=3, N_alpha=8):
    ...
    s_t = (
        torch.tensor(s_vals, dtype=torch.float32, device=a_b.device)
        .unsqueeze(0)
        .repeat(batch_size, 1)
    )
    alpha_t = (
        torch.tensor(alpha_vals, dtype=torch.float32, device=a_b.device)
        .unsqueeze(0)
        .repeat(batch_size, 1)
    )
